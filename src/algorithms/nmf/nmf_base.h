...
#ifndef GPUMLIB_NMF_BASE_H
#define GPUMLIB_NMF_BASE_H
...
#include "../../cuda/nmf/nmf_kernels.h"
...
namespace GPUMLib {
	...
	private:
		...
		static void Randomize(HostMatrix<cudafloat> & m) {
			size_t ne = m.Elements();
			cudafloat * p = m.Pointer();

			for (size_t e = 0; e < ne; e++) p[e] = ((cudafloat)rand()) / RAND_MAX + SMALL_VALUE_TO_ADD_DENOMINATOR;
		}
	...
	protected:
		...
		NMF(HostMatrix<cudafloat> & v, int r) : d_quality(1), V(v), WH(v.Rows(), v.Columns(), ColumnMajor) {
			assert(!v.IsRowMajor());
		...
		NMF(HostMatrix<cudafloat> & v, HostMatrix<cudafloat> & w, HostMatrix<cudafloat> & h) : d_quality(1), V(v), W(w), H(h), WH(v.Rows(), v.Columns(), ColumnMajor) {
			assert(!v.IsRowMajor());
			assert(!w.IsRowMajor());
			assert(!h.IsRowMajor());
