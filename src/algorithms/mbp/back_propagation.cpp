...
#include "../../cuda/mbp/mbp_kernels.h"
...
namespace GPUMLib {
	...
	void BackPropagation::Layer::Fire(cudaStream_t stream) {
		...
		if (isOutputLayer) {
			...
			if (connections > CUDA_MAX_THREADS_PER_BLOCK) {
				...
				do {
					int patternsToProcess = (patterns > CUDA_MAX_GRID_X_DIM) ? CUDA_MAX_GRID_X_DIM : patterns;
					gpumlib_cuda_mbp_fire_layer(patternsToProcess, dimInputsNeurons, sharedMemFire, stream, d_inputs + (processed * inputsWithoutBias), d_weights.Pointer(), (d_m != nullptr) ? d_m + (processed * totalNeuronsWithSelectiveActivation) : nullptr, mOffset, totalNeuronsWithSelectiveActivation, d_outputs.Pointer() + (processed * neurons));
					processed += patternsToProcess;
				} while (processed < patterns);
			}
		}
	...
	void BackPropagation::Layer::CalculateLocalGradient(cudaStream_t stream, cudafloat * rms, cudafloat * bestRMS, cudafloat rmsGrowToApplyRobustLearning, Layer & nextLayer) {
		...
		do {
			int patternsToProcess = (patterns > CUDA_MAX_GRID_X_DIM) ? CUDA_MAX_GRID_X_DIM : patterns;
			gpumlib_cuda_mbp_calculate_local_gradients(patternsToProcess, dimOutputsNeurons, sharedMemGradients, stream, rms, bestRMS, rmsGrowToApplyRobustLearning, d_outputs.Pointer() + (processed * neurons), nextLayer.d_weights.Pointer(), (d_m != nullptr) ? d_m + (processed * totalNeuronsWithSelectiveActivation) : nullptr, mOffset, totalNeuronsWithSelectiveActivation, nextLayer.d_localGradient.Pointer() + (processed * dimOutputsNeurons.x), nextLayerNeurons, neurons, d_localGradient.Pointer() + (processed * neurons), (d_m == nullptr) ? nullptr : d_localGradSpaceNet + (processed * totalNeuronsWithSelectiveActivation));
			processed += patternsToProcess;
		} while (processed < patterns);
	}
	...
	void BackPropagation::Layer::Init(int neurons, int inputs, int nextLayerNeurons, cudafloat initialLearningRate, cudafloat * layerInputs, bool isOutputLayer, cudafloat * m, cudafloat * localGradSpaceNet, int mOffset) {
		...
		inputsBlockSize = 1;
		while (inputsBlockSize < CUDA_MAX_THREADS_PER_BLOCK && inputsBlockSize < inputs) inputsBlockSize <<= 1;
		...
		MakeSureBlockDoesNotHaveTooMuchThreads(dimOutputsNeurons);

		sharedMemFire = connections * sizeof(cudafloat);
		sharedMemGradients = (dimOutputsNeurons.x * (dimOutputsNeurons.y + 1)) * sizeof(cudafloat);
	...
	void BackPropagation::SelectiveInputLayer::RandomizeWeights(cudafloat minValue, cudafloat maxValue, cudafloat initialLearningRate, HostArray<bool> & selectiveInputs) {
		...
		int ninputs = (int)selectiveInputs.Length();
	...
	void BackPropagation::SelectiveInputLayer::Fire(cudaStream_t stream) {
		...
		do {
			int patternsToProcess = (patterns > CUDA_MAX_GRID_X_DIM) ? CUDA_MAX_GRID_X_DIM : patterns;
			gpumlib_cuda_mbp_fire_selective_inputs(patternsToProcess, neurons, stream, d_inputs + (processed * neurons), d_weights.Pointer(), d_bias.Pointer(), d_outputs.Pointer() + (processed * neurons), neurons);
			processed += patternsToProcess;
		} while (processed < patterns);
	}
	...
	void BackPropagation::SelectiveInputLayer::CalculateLocalGradient(cudaStream_t stream, cudafloat * rms, cudafloat * bestRMS, cudafloat rmsGrowToApplyRobustLearning, Layer & nextLayer) {
		...
		do {
			int patternsToProcess = (patterns > CUDA_MAX_GRID_X_DIM) ? CUDA_MAX_GRID_X_DIM : patterns;
			gpumlib_cuda_mbp_calculate_local_gradients_selective_inputs(patternsToProcess, dimOutputsNeurons, sharedMemGradients, stream, rms, bestRMS, rmsGrowToApplyRobustLearning, d_inputs + (processed * neurons), d_weights.Pointer(), d_bias.Pointer(), nextLayer.d_weights.Pointer(), nextLayer.d_localGradient.Pointer() + (processed * nextLayerNeurons), nextLayerNeurons, neurons, d_localGradient.Pointer() + (processed * neurons));
			processed += patternsToProcess;
		} while (processed < patterns);
	}
	...
	void BackPropagation::CreateNetwork(HostArray<int> & sizeLayers, HostArray<int> * sizeSpaceLayers, HostArray<bool> * selectiveNeurons, HostMatrix<cudafloat> & trainInputPatterns, HostMatrix<cudafloat> & trainDesiredOutputPatterns, cudafloat initialLearningRate) {
		...
		assert(Layer::patterns > 0 && Layer::patterns == (int)trainDesiredOutputPatterns.Rows());
		...
		int numberSpaceLayers = (sizeSpaceLayers == nullptr) ? 0 : (int)sizeSpaceLayers->Length();
		...
		//Create the layers
		int numberLayers = (int)sizeLayers.Length() - 1;
		...
		int sizeRMSvector = (layers[outputLayer].connections > CUDA_MAX_THREADS_PER_BLOCK) ? Layer::patterns * layers[outputLayer].neurons : Layer::patterns;
		...
		//Other stuff
		patternsBlockSize = 1;
		while (patternsBlockSize < CUDA_MAX_THREADS_PER_BLOCK && patternsBlockSize < Layer::patterns) patternsBlockSize <<= 1;
	...
	HostArray<cudafloat> BackPropagation::GetLayerWeights(int layer) {
		assert(layer >= 0 && layer < (int)layers.Length());
	...
	void BackPropagation::SetLayerWeights(int layer, HostMatrix<cudafloat> & weights, HostArray<cudafloat> & bias) {
		...
		int inputs = (int)weights.Columns();

		assert(neurons == (int)bias.Length());
	...
	void BackPropagation::RandomizeWeights(cudafloat minValue, cudafloat maxValue) {
		int nSpaceLayers = (int)spaceLayers.Length();
	...
	void BackPropagation::Fire() {
		...
		int numLayers = (int)layers.Length();
	...
	cudafloat BackPropagation::GetRMS() {
		...
		// Calculate the RMS
		KernelCalculateRMS(streamKernels, patternsBlockSize, d_rms.Pointer(), d_rmsOut.Pointer(), (int)d_rms.Length(), numberPatternsNeurons);
	...
	void BackPropagation::TrainOneEpoch() {
		...
		if (robustLearning) {
			...
			gpumlib_cuda_mbp_robust_learning(maxNumberWeigths, streamKernels, d_rmsOut.Pointer(), d_bestRMS.Pointer(), (cudafloat)rmsGrowToApplyRobustLearning, layersRobustTraining, d_numberWeightsLayer.Pointer(), d_weightsLayers.Pointer(), d_bestWeightsLayers.Pointer(), d_learnRatesLayers.Pointer(), robustFactor, d_lastDeltaWithoutLMlayers.Pointer(), d_lastDeltaLayers.Pointer());
		} else {
			if (cudaStreamQuery(streamRMS) == cudaSuccess) {
				KernelCalculateRMS(streamRMS, patternsBlockSize, d_rms.Pointer(), d_rmsOut.Pointer(), (int)d_rms.Length(), numberPatternsNeurons);
	...
	void BackPropagation::Train(int epochs, cudafloat rmsStop) {
		for (int e = 0; e < epochs; e++) {
			TrainOneEpoch();
			if (rmsStop != cudafloat(0.0) && GetRMSestimate() <= rmsStop) {
				if (GetRMS() <= rmsStop) break;
			}
		}
	}
	...
	HostMatrix<cudafloat> BackPropagation::GetOutputs(HostMatrix<cudafloat> & inputs) {
		...
		int numberLayers = (int)layers.Length();
		int numberSpaceLayers = (int)spaceLayers.Length();
		...
		if (selectiveInputLayerSpaceNetwork != nullptr) {
			...
			do {
				...
				gpumlib_cuda_mbp_fire_selective_inputs(patternsToProcess, ninputs, streamKernels, layerInputs + (processed * ninputs), selectiveInputLayerSpaceNetwork->d_weights.Pointer(), selectiveInputLayerSpaceNetwork->d_bias.Pointer(), outputsSelectiveInput.Pointer() + (processed * ninputs), ninputs);
				processed += patternsToProcess;
			} while (processed < patterns);
		...
		for (int l = 0; l < numberSpaceLayers; l++) {
			if (spaceLayers[l].connections > CUDA_MAX_THREADS_PER_BLOCK) {
				dim3 dimNeuronsPatterns;
				...
				do {
					int patternsToProcess = (patterns > CUDA_MAX_GRID_X_DIM) ? CUDA_MAX_GRID_X_DIM : patterns;
					gpumlib_cuda_mbp_fire_layer(patternsToProcess, spaceLayers[l].dimInputsNeurons, spaceLayers[l].sharedMemFire, streamKernels, layerInputs + (processed * spaceLayers[l].inputsWithoutBias), spaceLayers[l].d_weights.Pointer(), nullptr, 0, Layer::totalNeuronsWithSelectiveActivation, spaceLayerOutputs[l]->Pointer() + (processed * spaceLayers[l].inputsWithoutBias));
					processed += patternsToProcess;
				} while (processed < patterns);
			}
		...
		if (selectiveInputLayer != nullptr) {
			...
			do {
				int patternsToProcess = (patterns > CUDA_MAX_GRID_X_DIM) ? CUDA_MAX_GRID_X_DIM : patterns;
				gpumlib_cuda_mbp_fire_selective_inputs(patternsToProcess, ninputs, streamKernels, layerInputs + (processed * ninputs), selectiveInputLayer->d_weights.Pointer(), selectiveInputLayer->d_bias.Pointer(), outputsSelectiveInput.Pointer() + (processed * ninputs), ninputs);
				processed += patternsToProcess;
			} while (processed < patterns);
		...
		for (int l = 0; l < numberLayers; l++) {
			if (layers[l].connections > CUDA_MAX_THREADS_PER_BLOCK) {
				dim3 dimNeuronsPatterns;
				...
				do {
					int patternsToProcess = (patterns > CUDA_MAX_GRID_X_DIM) ? CUDA_MAX_GRID_X_DIM : patterns;
					gpumlib_cuda_mbp_fire_layer(patternsToProcess, layers[l].dimInputsNeurons, layers[l].sharedMemFire, streamKernels, layerInputs + (processed * layers[l].inputsWithoutBias), layers[l].d_weights.Pointer(), (layers[l].d_m != nullptr) ? d_m + (processed * Layer::totalNeuronsWithSelectiveActivation) : nullptr, layers[l].mOffset, Layer::totalNeuronsWithSelectiveActivation, layerOutputs[l]->Pointer() + (processed * layers[l].inputsWithoutBias));
					processed += patternsToProcess;
				} while (processed < patterns);
			}
