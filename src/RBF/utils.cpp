...
#include "utils.h"
#include "../cuda/rbf/rbf_kernels.h"
...
namespace UTILS {
	...
	/*void printM(std::string desc, HostMatrix<float> Mat, int Rows){
		...
		printM(desc, Mat, false, (int)Mat.Rows());
		}

		void printM(std::string desc, HostMatrix<float> Mat, bool Order){
		printM(desc, Mat, Order, (int)Mat.Rows());
		}
	...
	DeviceMatrix<float> pseudoinverse(DeviceMatrix<float> &Input) {
		...
		int M = (int)Input.Rows();
		int N = (int)Input.Columns();
		...
		KernelSigmaInverse(d_Sigma.Pointer(), (int)d_Sigma.Rows(), (int)d_Sigma.Columns(), S.Pointer());
