...
namespace GPUMLib {
	...
	private:
		...
		public:
			...
			bool Resize(int I, int J) {
				if (w.ResizeWithoutPreservingData(J, I) != (size_t)J * I) return false;
				if (a.ResizeWithoutPreservingData(I) != (size_t)I) return false;
				if (b.ResizeWithoutPreservingData(J) != (size_t)J) return false;
	...
	public:
		...
		bool Init(int miniBatchSize, int cd) {
			...
			if (w.ResizeWithoutPreservingData(J, I) != (size_t)numberWeights) return false;
			if (a.ResizeWithoutPreservingData(I) != (size_t)I) return false;
			if (b.ResizeWithoutPreservingData(J) != (size_t)J) return false;
			...
			if (h_data.ResizeWithoutPreservingData(samples, J) != (size_t)samples * J) return false;
			if (h_recon.ResizeWithoutPreservingData(samples, J) != h_data.Elements()) return false;
			if (v_recon.ResizeWithoutPreservingData(samples, I) != (size_t)samples * I) return false;
			...
			if (randomValues.ResizeWithoutPreservingData(numberRandomValues) != (size_t)numberRandomValues) return false;

			RandomizeWeights();

			if (errors.ResizeWithoutPreservingData(I) != (size_t)I) return false;
		...
		RBM(int visibleUnits, int hiddenUnits, HostMatrix<cudafloat> & inputs, cudafloat initialLearningRate, cudafloat momentum = DEFAULT_MOMENTUM, bool useBinaryValuesVisibleReconstruction = false, cudafloat stdWeights = STD_WEIGHTS, int proportionRandomValuesGenerated = 1 /*, curandGenerator_t randomGenerator = nullptr*/) : mse(CUDA_VALUE(1.0)) {
			assert((size_t)visibleUnits == inputs.Columns() && inputs.IsRowMajor());
			assert(proportionRandomValuesGenerated > 0);

			v = inputs;

			_Init(visibleUnits, hiddenUnits, (int)inputs.Rows(), stdWeights, initialLearningRate, momentum, useBinaryValuesVisibleReconstruction, proportionRandomValuesGenerated, nullptr);
		}
