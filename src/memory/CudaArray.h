...
#ifndef GPUMLIB_CUDA_ARRAY_H
#define GPUMLIB_CUDA_ARRAY_H
...
namespace GPUMLib {
	...
	//! \attention The data on the host might differ from the data on the device. Use UpdateDevice and UpdateHost to syncronize data.
	template <class Type> class CudaArray {
	private:
		HostArray<Type> h;
		DeviceArray<Type> d;

		void TransferOwnerShipFrom(BaseArray<Type> & other) {
			if (this != other) {
