...
#include "../../cuda/svm/svm_kernel_type.h"
...
#define BUFFERING_BLOCK_SIZE (1<<20)
	...
	*/
	void readDataSet(FILE *f, GPUMLib::HostMatrix<cudafloat> & samples, GPUMLib::HostArray<int> & classes, int ncols, int positive_class) {
		...
		cout << "read dataset with " << row << " samples and " << ncols - 1 << " features" << endl;
	...
	void saveModel(char * model_filename, GPUMLib::HostMatrix<cudafloat> &model, cudafloat b) {
		...
		if (model_file.is_open()) {
			...
			model_file << model.Rows() << endl;
			//second line gives the amount of features
			model_file << model.Columns() - 2 << endl;
			//third line the hyperplane offset
			model_file << b << endl;
			//the remaining lines are in the form:
			//alpha_i | class_i | attribute_0 ... attribute_n-1
			for (int sv_i = 0; sv_i < (int)model.Rows(); sv_i++) {
				for (int col = 0; col < (int)model.Columns(); col++) {
					model_file << model(sv_i, col);
					if (col < (int)model.Columns() - 1)
						model_file << " ";
				}
				model_file << endl;
			}
			cout << "model saved to file " << model_filename << endl;
			model_file.close();
		} else
	...
	bool readModel(char * model_filename, int &n_sv, int &ndims,
		...
		if (model_file.is_open()) {
			...
			model_file >> h_b;
			//create the model
			model.ResizeWithoutPreservingData(n_sv, ncols);
			for (int row = 0; row < (int)model.Rows(); row++) {
				for (int col = 0; col < (int)model.Columns(); col++) {
					cudafloat val;
					model_file >> val;
					model(row, col) = val;
			...
			model_file.close();
			cout << "read model from file " << model_filename << " with " << n_sv << " SVs and " << ndims << " features" << endl;
			return true;
		} else {
	...
	void selectFastestDevice() {
		...
		if (num_devices > 0) {
			...
			for (device = 0; device < num_devices; device++) {
				...
				int numberOfCores = _ConvertSMVer2Cores(properties.major, properties.minor) * properties.multiProcessorCount;

				cout << "found device " << device << " : " << properties.name << " with " << numberOfCores << " CUDA cores" << endl;

				if (maxNumOfCores < numberOfCores) {
					maxNumOfCores = numberOfCores;
					fastest_device = device;
				}
			}
			// show info about the device
			cudaDeviceProp properties;
			cudaGetDeviceProperties(&properties, fastest_device);
			cout << "using CUDA device : " << fastest_device << " : " << properties.name << endl;
	...
	int main(int argc, char **argv) {
		...
		int amount_threads = CUDA_MAX_THREADS_PER_BLOCK;
		...
		size_t numberArguments = settings.getAmountArguments();
		//go through all arguments
		for (size_t i = 0; i < numberArguments; i++) {
			Argument* a = settings.getArgument(i);

			//			cout << a->argument << " ; " << (a->value == NULL ? "NULL" : a->value) << endl;
		...
		cout << "Negative class ID <> " << positive_class << endl;
		cout << "Positive class ID  = " << positive_class << endl;

		cout << "C negative = " << constant_c_negative << endl;
		cout << "C positive = " << constant_c_positive << endl;
		...
		if (train_model) {
			...
			// create the array for storing the alphas
			...
			cout << "training took " << t1 - t0 << " s" << endl;
		...
		if (classify_dataset) {
			...
			// if requested load model from file
			if (model_filename != NULL && !train_model) {
				if (!readModel(model_filename, n_sv, ndims, h_b, h_model)) {
					cout << "error while reading model" << endl;
					return -1;
				}
			}
			cout << "using model with " << n_sv << " SVs and " << ndims << " features" << endl;
			...
			//			cout << "debugging testing dataset" <<endl;
			//			for(int i=0;i<testing_dataset_size;i++){
			//				cout << h_testing_samples(i,0) << " " << h_testing_samples(i,1) << " : " << h_testing_classes[i] << endl;
			//			}
			...
			double t1 = shrDeltaT(1);
			cout << "classification took " << t1 - t0 << " s" << endl;

			//			 		for (int i=0;i<h_testing_results.Length();i++){
			//						cout << "target: " << h_testing_classes[i] << " predicted: " << h_testing_results[i] << endl;
			//			 		}

			//			cudafloat temp_classes[10];
			//			for(int i=0;i<h_testing_results.Length();i++){
			//				temp_classes[i]=h_testing_results[i];
			//			}
