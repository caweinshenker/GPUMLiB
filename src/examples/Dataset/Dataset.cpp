	...
	while (!inFile.eof()) {
		...
		while (iss >> aux) {
			...
			if (col == N) {
				...
				Y(row, 0) = (cudafloat)Classes[aux];

			} else {
				X(row, col) = (cudafloat)atof(aux.c_str());
			}
...
void normalize(HostMatrix<float> &X) {
	...
	for (int j = 0; j < (int)X.Columns(); j++) {
		...
		for (int i = 0; i < (int)X.Rows(); i++) {
			...
			normalized(i, j) = (cudafloat)((X(i, j) - mean) / std);
...
HostMatrix<float> crossvalidationTest(HostMatrix<float> &X, int folds, int fold_number) {
	...
	int adjust = 0;
	if (fold_number <= (int)X.Rows() % folds) {
		adjust += 1;
	}
	...
	for (int i = fold_number - 1; i < (int)X.Rows(); i += folds) {
...
HostMatrix<float> crossvalidationTrain(HostMatrix<float> &X, int folds, int fold_number) {
	...
	for (int i = 0; i < (int)X.Rows();) {
		...
		for (int j = 0; i < (int)X.Rows() && j < (int)X.Columns(); j++) {
			output(n, j) = X(i, j);
		}
