...
namespace GPUMLib {
	...
	__global__ void cuda_nmf_additive_euclidean_update_matrix(cudafloat * X, cudafloat * deltaX1, cudafloat * deltaX2, int elements) {
	...
	__global__ void cuda_nmf_multiplicative_euclidean_update_matrix(cudafloat * nm, cudafloat * dm, cudafloat * m, int elements) {
		int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
#define WMATRIX(_ROW, _COL, _N, _R) (W[(_COL) * (_N) + (_ROW)])
	...
	template <int blockSize> __global__ void SumW(cudafloat * W, int n, cudafloat * sumW) {
	...
	template <int blockSize> __global__ void SumH(cudafloat * H, int m, cudafloat * sumH) {
...
#define SVW(_R, _C) svw[(_R)][(_C)]
	...
	__global__ void cuda_nmf_multiplicative_divergence_update_matrix_w(cudafloat * W, cudafloat * H, cudafloat * V, cudafloat * WH, cudafloat * sumH, int n, int m, int r) {
	...
	__global__ void cuda_nmf_multiplicative_divergence_update_matrix_h(cudafloat * H, cudafloat * W, cudafloat * V, cudafloat * WH, cudafloat * sumW, int n, int m, int r) {
...
extern "C" cudaError_t gpumlib_cuda_nmf_additive_euclidean_update_matrix(unsigned int gridDim, unsigned int  blockDim, cudafloat * X, cudafloat * deltaX1, cudafloat * deltaX2, int elements) {
	GPUMLib::cuda_nmf_additive_euclidean_update_matrix<<<gridDim, blockDim>>>(X, deltaX1, deltaX2, elements);
...
extern "C" cudaError_t gpumlib_cuda_nmf_multiplicative_euclidean_update_matrix(unsigned int gridDim, unsigned int blockDim, cudafloat * nm, cudafloat * dm, cudafloat * m, int elements) {
	GPUMLib::cuda_nmf_multiplicative_euclidean_update_matrix<<<gridDim, blockDim>>>(nm, dm, m, elements);
...
extern "C" cudaError_t gpumlib_cuda_nmf_multiplicative_divergence_update_matrix_w(dim3 gridDim, dim3 blockDim, cudafloat * W, cudafloat * H, cudafloat * V, cudafloat * WH, cudafloat * sumH, int n, int m, int r) {
	GPUMLib::cuda_nmf_multiplicative_divergence_update_matrix_w<<<gridDim, blockDim>>>(W, H, V, WH, sumH, n, m, r);
...
extern "C" cudaError_t gpumlib_cuda_nmf_multiplicative_divergence_update_matrix_h(dim3 gridDim, dim3 blockDim, cudafloat * H, cudafloat * W, cudafloat * V, cudafloat * WH, cudafloat * sumW, int n, int m, int r) {
	GPUMLib::cuda_nmf_multiplicative_divergence_update_matrix_h<<<gridDim, blockDim>>>(H, W, V, WH, sumW, n, m, r);
