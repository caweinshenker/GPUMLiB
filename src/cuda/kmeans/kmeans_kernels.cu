...
EuclidianDistance(float* C, float* A, float* B, int wA, int wB, int wC, int hC) {
	...
	for (int a = aBegin, b = bBegin;
		...
		//     Csub += pow(AS(ty, k) - BS(tx, k),cudafloat(2.0));

		if ((a + 0) <= aEnd) Csub += pow(AS(ty, 0) - BS(tx, 0), cudafloat(2.0));
		if ((a + 1) <= aEnd) Csub += pow(AS(ty, 1) - BS(tx, 1), cudafloat(2.0));
		if ((a + 2) <= aEnd) Csub += pow(AS(ty, 2) - BS(tx, 2), cudafloat(2.0));
		if ((a + 3) <= aEnd) Csub += pow(AS(ty, 3) - BS(tx, 3), cudafloat(2.0));
		if ((a + 4) <= aEnd) Csub += pow(AS(ty, 4) - BS(tx, 4), cudafloat(2.0));
		if ((a + 5) <= aEnd) Csub += pow(AS(ty, 5) - BS(tx, 5), cudafloat(2.0));
		if ((a + 6) <= aEnd) Csub += pow(AS(ty, 6) - BS(tx, 6), cudafloat(2.0));
		if ((a + 7) <= aEnd) Csub += pow(AS(ty, 7) - BS(tx, 7), cudafloat(2.0));
		if ((a + 8) <= aEnd) Csub += pow(AS(ty, 8) - BS(tx, 8), cudafloat(2.0));
		if ((a + 9) <= aEnd) Csub += pow(AS(ty, 9) - BS(tx, 9), cudafloat(2.0));
		if ((a + 10) <= aEnd) Csub += pow(AS(ty, 10) - BS(tx, 10), cudafloat(2.0));
		if ((a + 11) <= aEnd) Csub += pow(AS(ty, 11) - BS(tx, 11), cudafloat(2.0));
		if ((a + 12) <= aEnd) Csub += pow(AS(ty, 12) - BS(tx, 12), cudafloat(2.0));
		if ((a + 13) <= aEnd) Csub += pow(AS(ty, 13) - BS(tx, 13), cudafloat(2.0));
		if ((a + 14) <= aEnd) Csub += pow(AS(ty, 14) - BS(tx, 14), cudafloat(2.0));
		if ((a + 15) <= aEnd) Csub += pow(AS(ty, 15) - BS(tx, 15), cudafloat(2.0));
...
__global__ void CenterAttribution(cudafloat *Output, int output_height, int output_width, int *attrib_center) {
...
__global__ void PrepareCenterCopy(cudafloat *Output, int output_height, int output_width, int *attrib_center) {
...
__global__ void CopyCenters(cudafloat *Output, int output_height, int output_width, cudafloat *Input, int input_width, int *attrib_center, cudafloat *Indexes, int idx_height, int idx_width) {
...
__global__ void reduce2(int *g_idata, int *g_odata, int *g_idata_old, unsigned int n) {
...
__global__ void CenterAttribution_Bounds(cudafloat *Output, int output_height, int output_width, int *attrib_center, float* upperbound) {
...
__global__ void CopyCenters2(cudafloat *Output, int output_height, int output_width, cudafloat *Input, int input_width, int *attrib_center) {
...
__global__ void FindS(cudafloat *Output, int output_height, int output_width, float *S) {
...
__global__ void Step3(float*Input, int input_height, float* UpperBounds, float* S, bool* R, int* CenterAttrib, float* LowerBounds, float* DistanceBeetweenCenters, float* InitialDistances, float* NewCenters, int centers_height, int centers_width) {
...
__global__ void Step5(int input_height, float* UpperBounds, bool* R, int* CenterAttrib, float* LowerBounds, float* DistanceBeetweenCenters, float* InitialDistances, float* NewCenters, int centers_height, int centers_width) {
...
__global__ void reduce_bool(bool *g_idata, bool *g_odata, unsigned int n) {
