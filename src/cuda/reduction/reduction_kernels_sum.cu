...
namespace GPUMLib {
	...
	template <int blockSize> __global__ void Sum(cudafloat * inputs, cudafloat * outputs, int numInputs) {
	...
	template <int blockSize> __global__ void SumSmallArray(cudafloat * inputs, cudafloat * output, int numInputs, cudafloat multiplyFactor) {
		extern __shared__ cudafloat sum[];
