...
namespace GPUMLib {
	...
	template <int blockSize> __global__ void Min(cudafloat * inputs, cudafloat * output, int numInputs) {
	...
	template <int blockSize> __global__ void MinIndex(cudafloat * inputs, cudafloat * output, int * indexes, int numInputs) {
		extern __shared__ cudafloat minvalue[];
	...
	template <int blockSize> __global__ void MinSmallArray(cudafloat * inputs, cudafloat * output, int numInputs) {
		extern __shared__ cudafloat minvalue[];
	...
	template <int blockSize> __global__ void MinSmallArrayIndex(cudafloat * inputs, cudafloat * output, int * minIndex, int numInputs, int * indexes) {
		extern __shared__ cudafloat minvalue[];
		...
		if (threadIdx.x < 32) {
			...
			if (threadIdx.x == 0) {
				if (output != nullptr) output[blockIdx.x] = minvalue[0];
