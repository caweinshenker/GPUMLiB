...
namespace GPUMLib {
	...
	__global__ void cuda_mbp_fire_layer(cudafloat * inputs, cudafloat * weights, cudafloat * m, int mOffset, int totalNeuronsWithSelectiveActivation, cudafloat * outputs) {
		extern __shared__ cudafloat iw[];
	...
	__global__ void cuda_mbp_fire_output_layer(cudafloat * inputs, cudafloat * weights, cudafloat * m, int mOffset, int totalNeuronsWithSelectiveActivation, cudafloat * desiredOutputs, cudafloat * outputs, cudafloat * localGradient, cudafloat * rms, cudafloat * localGradientSpaceNet) {
		extern __shared__ cudafloat iw[];
...
extern "C" cudaError_t gpumlib_cuda_mbp_fire_layer(unsigned int gridDim, dim3 blockDim, unsigned int sharedMemSize, cudaStream_t stream, cudafloat * inputs, cudafloat * weights, cudafloat * m, int mOffset, int totalNeuronsWithSelectiveActivation, cudafloat * outputs) {
	GPUMLib::cuda_mbp_fire_layer<<<gridDim, blockDim, sharedMemSize, stream>>>(inputs, weights, m, mOffset, totalNeuronsWithSelectiveActivation, outputs);

	return cudaGetLastError();
}


extern "C" cudaError_t gpumlib_cuda_mbp_fire_output_layer(unsigned int gridDim, dim3 blockDim, unsigned int sharedMemSize, cudaStream_t stream, cudafloat * inputs, cudafloat * weights, cudafloat * m, int mOffset, int totalNeuronsWithSelectiveActivation, cudafloat * desiredOutputs, cudafloat * outputs, cudafloat * localGradient, cudafloat * rms, cudafloat * localGradientSpaceNet) {
	GPUMLib::cuda_mbp_fire_output_layer<<<gridDim, blockDim, sharedMemSize, stream>>>(inputs, weights, m, mOffset, totalNeuronsWithSelectiveActivation, desiredOutputs, outputs, localGradient, rms, localGradientSpaceNet);
