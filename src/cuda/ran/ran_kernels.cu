...
#include "ran_kernels.h"

namespace GPUMLib {

	/* KERNEL Euclidian Distance */
	__global__ void EuclidianDistance(cudafloat *Output, int output_height, int output_width, cudafloat *Input, int input_width, cudafloat *Centers, int centers_width) {
	...
	__global__ void FindMinKernel(cudafloat *Output, int output_height, int output_width, float *min_array, int* min_idx, cudafloat* Targets) {
	...
	/********************/
	/* KERNEL Euclidian Distance */
	__global__ void FindNearestCenter(cudafloat *Output, int output_width, cudafloat *Sample, cudafloat *Centers, int centers_width, float* min_value) {
	...
	/****************/
	__global__ void ActivationMatrix(cudafloat *Output, int output_height, int output_width, cudafloat *Input, int input_width, cudafloat *Centers, int centers_width, float* c_width, float scalingfactor) {
		...
		if (idnx < output_width && idny < output_height) {
			...
			//column-major
			float value = exp(-(pow(sum, cudafloat(2.0)) / (scalingfactor*pow(c_width[idnx], cudafloat(2.0)))));
			...
			//row-major
			//Output[idnx + output_width * idny] = exp(-(pow(sum,cudafloat(2.0))/(scalingfactor*pow(c_width[idnx],cudafloat(2.0)))));
	...
	__global__ void SigmaInverse(float *Output, int output_height, int output_width, cudafloat *S) {
	...
	/************************/
	__global__ void CalculateDistance(cudafloat* Output, cudafloat* A, cudafloat* B, unsigned int n) {
		...
		sdata[tid] = (i < n) ? pow(A[i] - B[i], cudafloat(2.0)) : 0;
		if (i + blockDim.x < n)
			sdata[tid] += pow(A[i + blockDim.x] - B[i + blockDim.x], cudafloat(2.0));
...
#define BLOCK_SIZE 16
	...
	__global__ void CalculateNetworkActivation(cudafloat* output, cudafloat* Sample, int Length, cudafloat* dCenters, int NumCenters, cudafloat* dWeights, int NumClasses, cudafloat* dWidths, float scaling_factor) {
		...
		if (idnx < NumClasses && idny < NumCenters) {
			...
			//column-major
			//Output[idnx * output_height + idny] = exp(-(pow(sum,cudafloat(2.0))/(scalingfactor*pow(c_width[idnx],cudafloat(2.0)))));

			//row-major
			output[idnx + NumClasses * idny] = dWeights[idny*NumClasses + idnx] * exp(-(pow(sum, cudafloat(2.0)) / (scaling_factor*pow(dWidths[idny], cudafloat(2.0)))));
	...
	__global__ void UpdateWidths(cudafloat* dWidths, cudafloat* newWidths, int Length) {
	...
	__global__ void CalculateError(cudafloat* result, cudafloat* target, cudafloat* output, int Length) {

		int idnx = blockIdx.x*blockDim.x + threadIdx.x;

		if (idnx < Length) {

			output[idnx] = sqrt(pow(result[idnx] - target[idnx], cudafloat(2.0)));
	...
	__global__ void ReduceError(cudafloat* output, cudafloat* error, int Length) {

		float sum = 0;

		for (int i = 0; i < Length; i++) {
			sum += pow(output[i], cudafloat(2.0));
		}
	...
	__global__ void SumActivations(cudafloat* output, int Length, int NumCenters) {
	...
	__global__ void Copy(cudafloat* dst, cudafloat *src, int Length) {
