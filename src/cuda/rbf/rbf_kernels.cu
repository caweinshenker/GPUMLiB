...
__global__ void AdjustWidths(cudafloat *Distance, int distance_height, int distance_width, int rneighbours, float *widths) {
	...
	if (idny < distance_height) {
		...
		while (count <= rneighbours) {
			...
			if (min_tmp != -1) {
				width = width + pow(min_tmp, cudafloat(2.0));
				count = count + 1;
			}
...
ActivationMatrix(float* C, float* A, float* B, int wA, int wB, int wC, int hC, float scalingfactor, float* c_width) {
	...
	if (idnx < wC && idny < hC) {
		...
		C[c + hC * tx + ty] = exp(-(Csub / (scalingfactor*pow(c_width[idnx], cudafloat(2.0)))));
	}
}
