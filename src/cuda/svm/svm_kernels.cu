...
namespace GPUMLib {
	...
	__device__ int cuClassify(int sample_index, cudafloat * dataset, int dataset_size, cudafloat * model, int nsvs, cudafloat b, int ndims, cudafloat * kernel_args) {
		...
		for (int sv_i = 0; sv_i < nsvs; sv_i++) {
			// alpha is first in model, index 0
			cudafloat alpha_i = cuGetSampleAttribute(model, nsvs, sv_i, 0);
			// support vector class is second in model, index 1
			cudafloat y = cuGetSampleAttribute(model, nsvs, sv_i, 1);
			// remaining values correspond to the support vector coordinates
			...
			//			printf("sample_index %d sv_i %d alpha_i %f y %f k_proj %f sum %f\n",sample_index,sv_i,alpha_i, y,k_proj,sum);
		}
		sum += b;
		//		printf("[%d]=%f\n",sample_index,sum);
		if (sum > 0)
	...
	template<int blockSize>
	__global__ void cuCalculateB_FinalPass(cudafloat * data, int size) {
	...
	*/
	...
	template<int blockSize> __global__ void cuFirstOrderHeuristic1stPass(cudafloat * f, cudafloat * alphas, int * y, cudafloat * minimuns, int * min_indices,
	...
	template<int blockSize> __global__ void cuFirstOrderHeuristicFinalPass(cudafloat * minimuns_input, int * min_indices_input, cudafloat * maximuns_input,
	...
	void cudaInitializeSMO(int n_blocks, int n_threads_per_block, cudafloat * alphas, cudafloat * f, int * classes, int _nsamples) {
		cuInitializeSMO<<<n_blocks, n_threads_per_block>>>(alphas, f, classes, _nsamples);
	}
		...
		switch (kernel_type) {
			case SVM_KT_LINEAR:
				cuCalculateOffsetsUsingModel<SVM_KT_LINEAR><<<n_blocks, n_threads_per_block, 0, stream_bias_calculus>>>
					(results, model, nsvs, ndims, kernel_args);
				break;
			case SVM_KT_POLYNOMIAL:
		...
		switch (kernel_type) {
			case SVM_KT_LINEAR:
				cuClassifyDataSet<SVM_KT_LINEAR><<<n_blocks, n_threads_per_block>>>
					(results, dataset, dataset_size, model, nsvs, b, ndims, kernel_args);
				break;
			case SVM_KT_POLYNOMIAL:
				cuClassifyDataSet<SVM_KT_POLYNOMIAL><<<n_blocks, n_threads_per_block>>>
					(results, dataset, dataset_size, model, nsvs, b, ndims, kernel_args);
				break;
			case SVM_KT_RBF:
				cuClassifyDataSet<SVM_KT_RBF><<<n_blocks, n_threads_per_block>>>
					(results, dataset, dataset_size, model, nsvs, b, ndims, kernel_args);
				break;
			case SVM_KT_SIGMOID:
				cuClassifyDataSet<SVM_KT_SIGMOID><<<n_blocks, n_threads_per_block>>>
					(results, dataset, dataset_size, model, nsvs, b, ndims, kernel_args);
				break;
			case SVM_KT_UKF:
				cuClassifyDataSet<SVM_KT_UKF><<<n_blocks, n_threads_per_block>>>
					(results, dataset, dataset_size, model, nsvs, b, ndims, kernel_args);
				break;
		}
	}
