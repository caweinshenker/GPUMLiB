...
namespace GPUMLib {
	...
	float ResourceAllocatingNetwork::FindMaxWidth(DeviceMatrix<float> &device_X, DeviceMatrix<float> &Y){
		...
		KernelEuclidianDistance(device_output2.Pointer(), (int)device_output2.Rows(), (int)device_output2.Columns(), device_X.Pointer(), (int)device_X.Columns(), device_X.Pointer(), (int)device_X.Columns());

		DeviceArray<float> dMin(device_output2.Rows());
		DeviceArray<int> dIdx(device_output2.Rows());

		//Min
		FindMin(device_output2.Pointer(), (int)device_output2.Rows(), (int)device_output2.Columns(), dMin.Pointer(), dIdx.Pointer(), Y.Pointer());
	...
	void ResourceAllocatingNetwork::FindNearestCenter(cudafloat *Sample, int Length, float *value, int* index, cudafloat* output){
		KernelFindNearestCenter(output, NumCenters, Sample, dCenters.Pointer(), (int)dCenters.Columns(), value);
	}
	...
	void ResourceAllocatingNetwork::UpdateWeights(cudafloat *Sample, int Length, float* Target){
		...
		KernelActivationMatrix(device_output2.Pointer(), (int)device_output2.Rows(), (int)device_output2.Columns(),
			dIMemory.Pointer(), (int)dIMemory.Columns(),
			dCenters.Pointer(), (int)dCenters.Columns(),
			dWidths.Pointer(), overlap_factor);

		UTILS::pseudoinverse2(device_output2);

		matmul(dWeights.Pointer(), device_output2.Pointer(), dTMemory_2.Pointer(), (int)device_output2.Rows(), (int)dTMemory_2.Columns(), (int)dTMemory_2.Columns(), (int)device_output2.Columns());
	}
