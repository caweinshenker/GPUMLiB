...
#include "../cuda/svm/svm_kernels.h"
	...
	public:
		...
		**/
		void calculateB_1stPass(cudaStream_t stream,
			...
			cudafloat * results,
			int n_svs) {
			cudaCalculateB_1stPass(stream, blocks, blockSize, offsets, results, n_svs);
		}
		...
		**/
		void calculateB_FinalPass(cudaStream_t stream, int blockSize, cudafloat * input_floats, int input_size) {
			cudaCalculateB_FinalPass(stream, blockSize, input_floats, input_size);
		}
		...
		**/
		void kernelFirstOrderHeuristic1stPass(cudaStream_t stream, int blocks, int blockSize,
			...
			cudafloat constant_epsilon, cudafloat constant_c) {
				cudaFirstOrderHeuristic1stPass(stream, blocks, blockSize,
													f, alphas,
													y, minimuns, min_indices,
													maximuns, max_indices, input_size,
													constant_epsilon, constant_c);
		}
		...
		**/
		void kernelFirstOrderHeuristicFinalPass(cudaStream_t stream, int blockSize,
			...
			int input_size) {
			cudaFirstOrderHeuristicFinalPass(stream, blockSize,
												  minimuns_input, min_indices_input,
												  maximuns_input, max_indices_input,
												  input_size);
		}
		...
		**/
		void updateAlphas(cudaStream_t stream, GPUMLib::svm_kernel_type kernel_type,
			...
			GPUMLib::DeviceArray<cudafloat> &d_kernel_args, int training_dataset_size, int ndims) {
				cudaUpdateAlphasAdvanced(stream, kernel_type,
										  d_x, d_alphas,
										  d_y, constant_c_negative, constant_c_positive,
										  d_kernel_args, training_dataset_size, ndims);
		}
		...
		**/
		void updateKKTConditions(cudaStream_t stream, GPUMLib::svm_kernel_type kernel_type, int n_blocks, int blocksize,
			...
			cudaUpdateKKTConditions(stream, kernel_type, n_blocks, blocksize,
										 d_f, d_y,
										 d_x, d_kernel_args,
		...
		**/
		void runSMO(GPUMLib::HostMatrix<cudafloat> & h_x, GPUMLib::HostArray<int> & h_y,
			...
			int training_dataset_size = (int)h_x.Rows();
			int ndims = (int)h_x.Columns();

			cout << "started Sequential Minimum Optimization algorithm" << endl;
			...
			if (i_high < 0 || i_low < 0) {
				cout << "error: couldn't initialize SMO's indices.." << endl;
				assert(i_high >= 0);
				assert(i_low >= 0);
			}
			...
			cudaInitializeSMO(n_blocks, n_threads_per_block, d_alphas.Pointer(), d_f.Pointer(), d_y.Pointer(), training_dataset_size);
			checkCUDA_Errors();

			GPUMLib::CopyLowHighIndicesToDeviceMemory(i_low, i_high, h_b_low, h_b_high);
			...
			for (;;) {
				if (convergence_checker >= 8){
					...
					if (!memory_transaction_requested) {
						GPUMLib::CopyLowHighOffsetsFromDeviceMemory(stream_memory_transaction, i_low, i_high, h_b_low, h_b_high);
		...
		**/
			...
			cudafloat &h_b)
		{
			...
			int training_dataset_size = (int)h_samples.Rows();
			...
			int ndims = (int)h_samples.Columns();
			...
			cudaCalculateOffsetsUsingModel(kernel_type, n_blocks, n_threads_per_block, stream_bias_calculus,
		...
		**/
			...
			GPUMLib::HostArray<int> &h_testing_results)
		{
			...
			int testing_dataset_size = (int)h_testing_samples.Rows();
			...
			cudaClassifyDataSet(kernel_type, n_blocks, n_threads_per_block,
